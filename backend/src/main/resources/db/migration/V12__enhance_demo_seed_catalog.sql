...
CREATE TEMP TABLE seed_managed_roles (
...
VALUES
  ('inventory.lead', 'inventory.lead@seshop.local', '+84900000004', '$2a$10$FF27zESs/E9SWwgL.ocbEuLN29sbbUbHXmis1W3NhRszDF//2dHLG', 'STAFF', 'ACTIVE'),
  ('cashier.d1', 'cashier.d1@seshop.local', '+84900000005', '$2a$10$FF27zESs/E9SWwgL.ocbEuLN29sbbUbHXmis1W3NhRszDF//2dHLG', 'STAFF', 'ACTIVE'),
  ('fulfillment.lead', 'fulfillment.lead@seshop.local', '+84900000006', '$2a$10$FF27zESs/E9SWwgL.ocbEuLN29sbbUbHXmis1W3NhRszDF//2dHLG', 'STAFF', 'ACTIVE'),
  ('marketing.lead', 'marketing.lead@seshop.local', '+84900000007', '$2a$10$FF27zESs/E9SWwgL.ocbEuLN29sbbUbHXmis1W3NhRszDF//2dHLG', 'STAFF', 'ACTIVE'),
  ('support.agent', 'support.agent@seshop.local', '+84900000008', '$2a$10$FF27zESs/E9SWwgL.ocbEuLN29sbbUbHXmis1W3NhRszDF//2dHLG', 'STAFF', 'ACTIVE'),
  ('finance.manager', 'finance.manager@seshop.local', '+84900000009', '$2a$10$FF27zESs/E9SWwgL.ocbEuLN29sbbUbHXmis1W3NhRszDF//2dHLG', 'STAFF', 'ACTIVE')
...
VALUES
  ...
  ('LOCATION_SUPERVISOR', 'report.read')
ON CONFLICT DO NOTHING;
...
FROM (VALUES
  ('inventory.lead', 'INVENTORY_MANAGER', 'super.admin'),
  ('cashier.d1', 'CASHIER', 'super.admin'),
  ('fulfillment.lead', 'FULFILLMENT_STAFF', 'super.admin'),
  ('marketing.lead', 'MARKETING_MANAGER', 'super.admin'),
  ('support.agent', 'CUSTOMER_SUPPORT', 'super.admin'),
  ('finance.manager', 'FINANCE_MANAGER', 'super.admin')
...
INSERT INTO seed_extra_accessories (
  ...
  ('ACC-BELT-001', 'Vintage Black Leather Belt - One Size Black Leather', 'Unbranded', 'Belt', 'One Size', 'Black', 490000, 'Very Good', 'Unisex', 'Leather', '2000S', 'https://commons.wikimedia.org/wiki/Special:FilePath/Leather_belt.jpg?width=1200', 'Vintage Accessories'),
  ('ACC-BAG-001', 'Kate Spade Structured Leather Handbag - One Size Cream Leather', 'Kate Spade', 'Handbag', 'One Size', 'Cream', 3200000, 'Excellent', 'Womens', 'Leather', '2010S', 'https://commons.wikimedia.org/wiki/Special:FilePath/Kate_Spade_handbag.jpg?width=1200', 'Designer Accessories'),
  ('ACC-SCARF-001', 'Late 19th Century Silk Scarf - One Size Multicoloured Silk', 'Unbranded', 'Scarf', 'One Size', 'Multicoloured', 1250000, 'Good', 'Womens', 'Silk', 'Pre-1950S', 'https://commons.wikimedia.org/wiki/Special:FilePath/Scarf,_late_19th_century_(CH_18466163).jpg?width=1200', 'Vintage Accessories'),
  ('ACC-CAP-001', 'Festival Baseball Cap - One Size Blue Cotton', 'Unbranded', 'Cap', 'One Size', 'Blue', 380000, 'Very Good', 'Unisex', 'Cotton', '2010S', 'https://commons.wikimedia.org/wiki/Special:FilePath/Baseball_cap_(15930546668).jpg?width=1200', 'Vintage Accessories'),
  ('ACC-WATCH-001', 'Fossil Minimal Dial Wristwatch - One Size Brown Leather', 'Fossil', 'Watch', 'One Size', 'Brown', 2600000, 'Very Good', 'Unisex', 'Leather/Steel', '2010S', 'https://commons.wikimedia.org/wiki/Special:FilePath/Fossil_wristwatch_with_white_background.jpg?width=1200', 'Jewelry & Watches'),
  ('ACC-WALLET-001', 'Vintage Men''s Brown Leather Wallet - One Size Brown Leather', 'Unbranded', 'Wallet', 'One Size', 'Brown', 620000, 'Good', 'Mens', 'Leather', '2000S', 'https://commons.wikimedia.org/wiki/Special:FilePath/A_men%27s_wallet.jpg?width=1200', 'Vintage Accessories'),
  ('ACC-TOTE-001', 'QWSTION Black Flap Tote - One Size Black Canvas', 'QWSTION', 'Tote Bag', 'One Size', 'Black', 1800000, 'Excellent', 'Unisex', 'Canvas', '2010S', 'https://commons.wikimedia.org/wiki/Special:FilePath/QWSTION-FLAP-TOTE-MEDIUM-ALL-BLACK-FRONT.jpg?width=1200', 'Bags & Totes'),
  ('ACC-NECKLACE-001', 'Vintage Sapphire Accent Necklace - One Size Gold Metal', 'Unbranded', 'Necklace', 'One Size', 'Gold', 2100000, 'Very Good', 'Womens', 'Metal/Stone', '1990S', 'https://commons.wikimedia.org/wiki/Special:FilePath/Necklace_-_Meta_Overbeck_(27770427469).jpg?width=1200', 'Jewelry & Watches');

INSERT INTO categories (name, description)
SELECT DISTINCT category_name, 'Internet-sourced accessory seed category for ' || category_name || '.'
FROM seed_extra_accessories
...
SELECT
  s.product_name,
  s.brand,
  'Seeded from internet-sourced accessory imagery for demo merchandising. Condition: '
...
SELECT sv.variant_id, l.id, 2 + (sv.rn % 3), CASE WHEN sv.rn % 4 = 0 THEN 1 ELSE 0 END
...
INSERT INTO inventory_balances (variant_id, location_id, on_hand_qty, reserved_qty)
SELECT sv.variant_id, l.id, 1 + (sv.rn % 3), 0
FROM seeded_variants sv
JOIN locations l ON l.code = 'STORE-THAO-DIEN'
WHERE sv.rn BETWEEN 3 AND 8
ON CONFLICT (variant_id, location_id) DO UPDATE
SET on_hand_qty = EXCLUDED.on_hand_qty,
    reserved_qty = EXCLUDED.reserved_qty,
    updated_at = NOW();
...
FROM seeded_variants sv
JOIN locations l ON l.code = 'STORAGE-BT'
ON CONFLICT (variant_id, location_id) DO UPDATE
SET on_hand_qty = EXCLUDED.on_hand_qty,
    reserved_qty = EXCLUDED.reserved_qty,
    updated_at = NOW();
...
SELECT u.id, 'ROLE_PERMISSION_ASSIGNED', 'SeedData', 'V12_RBAC_MATRIX', '{"source":"V12__enhance_demo_seed_catalog","managedRoles":12}'::jsonb, TIMESTAMPTZ '2026-05-12 08:00:00+07'
FROM users u WHERE u.username = 'super.admin'
UNION ALL
SELECT u.id, 'PRODUCT_CREATED', 'SeedData', 'ACCESSORY_BATCH_2026_05', '{"source":"Wikimedia Commons","items":8}'::jsonb, TIMESTAMPTZ '2026-05-12 08:10:00+07'
FROM users u WHERE u.username = 'marketing.lead'
UNION ALL
SELECT u.id, 'INVENTORY_ADJUSTED', 'SeedData', 'MULTI_LOCATION_ACCESSORIES', '{"locations":["ONLINE-HCM","STORE-D1","STORE-THAO-DIEN","ONLINE-HN","STORAGE-BT","STORAGE-DN"]}'::jsonb, TIMESTAMPTZ '2026-05-12 08:20:00+07'
FROM users u WHERE u.username = 'inventory.lead';
